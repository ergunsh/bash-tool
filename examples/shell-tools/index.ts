/**
 * Example: Using Shell Tools with AI SDK
 *
 * Shell tools are custom commands that run inside the bash environment.
 * They use Zod schemas for validation and output JSON for composability
 * with Unix tools like jq.
 *
 * Run with: npx tsx examples/shell-tools/index.ts
 */
...
// Define shell tools with Zod schemas
...
async function main() {
  console.log("Creating bash tool with shell tools...\n");

  // Create bash tool with shell tools - they become available as CLI commands
