...
This example demonstrates where CLI tools provide the most benefit: when you have many related operations.
...
CLI tools consolidate all operations into a single bash tool with a compact description. The agent discovers available commands from the bash tool description.
...
Run both examples and compare the `Total tokens` output. The CLI tools approach should use fewer tokens because:

1. **Single tool definition**: One bash tool vs 15 separate tool schemas
2. **Compact descriptions**: CLI tools use a summarized format in the bash tool description
