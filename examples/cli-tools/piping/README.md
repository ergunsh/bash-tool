...
With CLI tools, the agent can do everything in a single command:
...
Compare:
- **Steps**: CLI tools should complete in fewer steps (1-2 vs 2-4)
- **Total tokens**: CLI tools should use fewer tokens (less LLM processing)

## Key Insight

CLI tools return rich, denormalized data that's optimized for piping. The `list-orders` tool embeds customer and product details in each order, enabling powerful one-liner queries with `jq`.
...
This ensures identical behavior between CLI tools and AI SDK tools, making the comparison fair. The only difference is how tools are exposed to the agent.
