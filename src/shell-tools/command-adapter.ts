...
import type { ShellToolDefinition, ShellToolsRecord } from "./types.js";

export { toKebabCase };

/**
 * Convert a ShellToolDefinition to a just-bash Command.
 *
 * The command:
 ...
 * @param definition - The shell tool definition
 */
...
/**
 * Convert a record of ShellToolDefinitions to an array of just-bash Commands.
 *
 * @param shellTools - Record of shell tool definitions keyed by camelCase name
 * @returns Array of just-bash Commands with kebab-case names
 */
export function toCommands(shellTools: ShellToolsRecord): Command[] {
  return Object.entries(shellTools).map(([name, definition]) =>
