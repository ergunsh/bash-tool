...
import { experimental_createShellTool } from "./shell-tool.js";
...
/**
 * Integration tests that verify shell tools work correctly
 * with the real just-bash sandbox environment.
 */
describe("shell tools integration", () => {
  describe("basic shell tool", () => {
    it("executes shell tool with args and returns JSON", async () => {
    ...
    it("shows help documentation on --help flag", async () => {
      const greet = experimental_createShellTool({
  ...
  describe("shell tool with context", () => {
    it("can access filesystem via context", async () => {
      const readConfig = experimental_createShellTool({
    ...
    it("can access environment variables via context", async () => {
      const getEnv = experimental_createShellTool({
    ...
    it("can access current working directory via context", async () => {
      const getCwd = experimental_createShellTool({
  ...
  describe("piping with jq", () => {
    it("can pipe shell tool output to jq", async () => {
      const listUsers = experimental_createShellTool({
    ...
    it("can filter array with jq", async () => {
      const getData = experimental_createShellTool({
  ...
  describe("multiple shell tools", () => {
    it("registers multiple shell tools", async () => {
      const fetchUser = experimental_createShellTool({
        description: "Fetches a user",
      ...
      const sendEmail = experimental_createShellTool({
      ...
      const { tools } = await createBashTool({
        shellTools: { fetchUser, sendEmail },
      });
  ...
  describe("shell tools prompt", () => {
    it("includes shell tools in bash tool description", async () => {
      const fetchUser = experimental_createShellTool({
        description: "Fetches a user from the database",
        inputSchema: z.object({
          id: z.string().describe("The user ID"),
  ...
  describe("error handling", () => {
    it("returns error when execute throws", async () => {
      const failingTool = experimental_createShellTool({
    ...
    it("handles unknown flags gracefully", async () => {
      const simpleTool = experimental_createShellTool({
