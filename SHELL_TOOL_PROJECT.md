...
interface ShellToolDefinition<TInput, TOutput> {
  /**
...
export const fetchUser = shellTool({
...
When shell tools are registered with `createBashTool`, the bash tool's system prompt includes inline signatures for all available tools.
...
const { tools } = await createBashTool({
  shellTools: [fetchUser, sendEmail, queryDb],
  files: {
...
function toCommand(definition: ShellToolDefinition): CustomCommand {
...
function toInlineSignature(definition: ShellToolDefinition): string {
...
function generateHelp(definition: ShellToolDefinition): string {
